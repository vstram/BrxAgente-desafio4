...
func TestComplianceChecker_CheckCompliance(t *testing.T) {
	...
	result, err := checker.CheckCompliance(validData)
	if err != nil {
		t.Fatalf("Erro ao verificar compliance: %v", err)
	}
	...
	if result.Score != 100.0 {
		t.Errorf("Esperado score 100.0, obtido %.2f", result.Score)
	}
}
...
func TestComplianceChecker_CheckCompliance_Violations(t *testing.T) {
	...
	result, err := checker.CheckCompliance(violationData)
	if err != nil {
		t.Fatalf("Erro ao verificar compliance: %v", err)
	}
	...
	if result.Score >= 100.0 {
		t.Errorf("Esperado score menor que 100, obtido %.2f", result.Score)
	}
}

func TestAuditTrail_RecordAction(t *testing.T) {
	auditTrail := NewAuditTrail(100, 30)
...
func BenchmarkRiskAssessor_AssessEmployeeRisk(b *testing.B) {
	auditTrail := NewAuditTrail(1000, 30)
	checker := NewComplianceChecker("../data/regulations")
	riskAssessor := NewRiskAssessor(checker, auditTrail)
